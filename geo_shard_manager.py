...
class GeoShardManager:
    ...
    def _init_shards(self):
        ...
        self.logger.info(
            f"Initialized {len(self.shards)} shards across {len(self.rings)} regions"
        )
    ...
    def route(self, record: GeoRecord) -> ShardInfo:
        ...
        if shard.load_factor() >= self.HOT_SPOT_THRESHOLD:
            if not shard.hot_spot:
                shard.hot_spot = True
                self.logger.warning(
                    f"🔥 Hot-Spot detected: {shard_id} ({shard.record_count:,} records)"
                )
    ...
    def split_hot_shard(self, shard_id: str) -> Tuple[str, str]:
        ...
        self.logger.info(f"✂️  Split {shard_id} → {shard_id} + {new_id}")
    ...
    def balance_score(self) -> float:
        counts = [s.record_count for s in self.shards.values()]
        if len(counts) < 2:
            return 1.0
